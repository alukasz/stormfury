...
-module(storm_dsl_lexer).

-export([string/1,string/2,token/2,token/3,tokens/2,tokens/3]).
-export([format_error/1]).

%% User code. This is placed here to allow extra attributes.
-file("src/storm_dsl_lexer.xrl", 31).
...
-file("src/storm_dsl_lexer.erl", 324).
yystate() -> 10.
...
-file("src/storm_dsl_lexer.xrl", 9).
yyaction_0(TokenChars, TokenLine) ->
     { token, { int, TokenLine, list_to_integer (TokenChars) } } .

-compile({inline,yyaction_1/2}).
-file("src/storm_dsl_lexer.xrl", 10).
yyaction_1(TokenChars, TokenLine) ->
     { token, { float, TokenLine, list_to_float (TokenChars) } } .

-compile({inline,yyaction_2/3}).
-file("src/storm_dsl_lexer.xrl", 13).
yyaction_2(TokenChars, TokenLen, TokenLine) ->
     { token, { string, TokenLine, list_to_binary (strip (TokenChars, TokenLen)) } } .

-compile({inline,yyaction_3/2}).
-file("src/storm_dsl_lexer.xrl", 16).
yyaction_3(TokenChars, TokenLine) ->
     { token, { list_to_atom (TokenChars), TokenLine } } .

-compile({inline,yyaction_4/1}).
-file("src/storm_dsl_lexer.xrl", 17).
yyaction_4(TokenLine) ->
     { token, { '..', TokenLine } } .

-compile({inline,yyaction_5/2}).
-file("src/storm_dsl_lexer.xrl", 20).
yyaction_5(TokenChars, TokenLine) ->
